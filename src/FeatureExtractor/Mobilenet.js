...
class Mobilenet {
     ...
     * @param {HTMLVideoElement || p5.Video || string || number} inputOrLabel
     * @param {string || number || function} labelOrCallback
     * @param {function} cb
     */
    async addImage(inputOrLabel, labelOrCallback, cb) {
        const {image, string, number, callback} = new ArgSeparator(this.video, inputOrLabel, labelOrCallback, cb);

        // label could be a string or a number
        // TODO: can clean up more
        let index = number;
        if (string !== undefined) {
            if (!this.mapStringToIndex.includes(string)) {
                index = this.mapStringToIndex.push(string) - 1;
            } else {
                index = this.mapStringToIndex.indexOf(string);
            }
        }
        if (index === undefined) {
            throw new Error("Must provide either a string label or the numeric index of that label.");
        }

        return callCallback(this.addImageInternal(image, index), callback);
    }
    ...
    async predict(inputOrCallback, cb) {
        const {image: imgToPredict, callback} = new ArgSeparator(this.video, inputOrCallback, cb);
    ...
    infer(input, endpoint = "conv_preds") {
        const {image} = new ArgSeparator(this.video, input);
        if (!image) {
            throw new Error("No input image found.");
        }
        return this.mobilenetInfer(image, endpoint);
    }
}
